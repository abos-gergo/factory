...
pub(crate) struct PlayerTableHandle<'ctx> {
...
pub(crate) trait PlayerTableAccess {
...
pub(crate) struct PlayerInsertCallbackId(__sdk::CallbackId);
pub(crate) struct PlayerDeleteCallbackId(__sdk::CallbackId);
...
pub(crate) struct PlayerUpdateCallbackId(__sdk::CallbackId);
...
/// Access to the `steam_id` unique index on the table `player`,
/// which allows point queries on the field of the same name
/// via the [`PlayerSteamIdUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.player().steam_id().find(...)`.
pub(crate) struct PlayerSteamIdUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Player, u64>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> PlayerTableHandle<'ctx> {
    /// Get a handle on the `steam_id` unique index on the table `player`.
    pub(crate) fn steam_id(&self) -> PlayerSteamIdUnique<'ctx> {
        PlayerSteamIdUnique {
...
impl<'ctx> PlayerSteamIdUnique<'ctx> {
    /// Find the subscribed row whose `steam_id` column value is equal to `col_val`,
    /// if such a row is present in the client cache.
    pub(crate) fn find(&self, col_val: &u64) -> Option<Player> {
...
pub(crate) struct PlayerNameUnique<'ctx> {
...
impl<'ctx> PlayerTableHandle<'ctx> {
    ...
    pub(crate) fn name(&self) -> PlayerNameUnique<'ctx> {
        PlayerNameUnique {
...
impl<'ctx> PlayerNameUnique<'ctx> {
    ...
    /// if such a row is present in the client cache.
    pub(crate) fn find(&self, col_val: &String) -> Option<Player> {
        self.imp.find(col_val)
    }
}
